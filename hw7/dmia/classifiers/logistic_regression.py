...
class LogisticRegression:
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        y_proba = 1.0 / (1.0 + np.exp(-X.dot(self.w)))
    ...
    def predict(self, X):
        ...
        y_pred = y_proba.round().astype(np.uint8)
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        m = y_batch.size
        X_batch = X_batch.A
        h = 1.0 / (1.0 + np.exp(-np.dot(self.w, X_batch.T)))
        loss += -(y_batch * np.log(h) + (1-y_batch) * np.log(1.0 - h)).mean()
        dw += ((h - y_batch).reshape((1, m)).repeat(dw.size, 0) * X_batch.T).mean(1)
        ...
        loss += reg * (self.w[:-1]*self.w[:-1]).sum() / (2*m)
        dw[:-1] += reg * self.w[:-1] / m
